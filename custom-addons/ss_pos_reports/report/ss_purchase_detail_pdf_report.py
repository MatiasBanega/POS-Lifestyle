...
class SSPurchaseDetailsReportGst(models.AbstractModel):    
    _name = 'report.ss_pos_reports.sspurchase_detail_wise'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        sql = ''' 
      select grn_no,bill_no,grn_date , partner_id, ware_house, sub_total,
        tax_amt,cess, total_val, add_ded, tcs_amt,net_amt,freight_charges, pcode, product_id,brand,qty,mrp,gkm,sp_margin_mrp,
        m_down,diff_margin,diff_amt,lcost,line_subtot,dept,categ,manuftr,tax,igst,cgst,sgst,line_taxtot,
        cess_tot,created,reversal_no,reversed_date from ss_purchase_detail_report_screen_line
                where ss_purchase_order_id=(select max(ss_purchase_order_id) from ss_purchase_detail_report_screen_line)    
