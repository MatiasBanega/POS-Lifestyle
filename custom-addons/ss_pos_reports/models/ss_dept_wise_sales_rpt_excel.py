...
class ss_department_wise_sales_details_wzd(models.Model):
    _name = "ss.dept.wise.sales.report"
...
#         seq = 0
        for line in get_dept_lines(self):
            ...
            ss_dept_wise_sale_line.append((0,0,{
...
#                'product_id' : self.product_id.name,
        ...
        res = self.env['ir.model.data'].check_object_reference(
                                            'ss_pos_reports', 'view_dept_wise_sale_ss')
        return {
                    'name': 'dept wise sales Report',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'res_model': 'ss.dept.screen.wzd',
                    'domain': [],
...
class ss_dept_sale_screen_wzd(models.Model):
    _name = "ss.dept.screen.wzd"
    _description = "Department Wise Sales Reports"
    
    name = fields.Char(default="Dept Wise Sales Report")
    ss_dept_wise_sale_line = fields.One2many('ss.dept.wise.sales.line','ss_deptsale_id')
    start_date = fields.Date('Date From')
    end_date = fields.Date('Date To')
    company_id = fields.Char('Company')

    
    def print_ss_dept_orders_excel_report(self):
        ...
        sheet= workbook.add_sheet('Department Wise Sales Report')
        ...
        sheet.write_merge(0, 1, 0, 7, 'Dept Wise Sales Report',header)
...
#         
        ...
        fp =io.BytesIO()
        workbook.save(fp)
        export_id = self.env['ss.excel.extended.dept.rep'].create({'excel_file': base64.encodestring(fp.getvalue()), 'file_name': filename})
...
ss_dept_sale_screen_wzd()
 
class ss_dept_wise_sale_screen_line(models.Model):
    _name = "ss.dept.wise.sales.line"
    _description = "Sales summary Line"
    
    ss_deptsale_id = fields.Many2one('ss.dept.screen.wzd',string='ss_deptsale_id',ondelete='cascade')
...
ss_dept_wise_sale_screen_line()
    
     
class ss_excel_extended_orders_rep(models.Model):
    _name= "ss.excel.extended.dept.rep"
