...
union all
 ...
 SELECT 'B2B - Return' as terminal,
 sum(( select count(1) from c_invoice where c_invoice_id=c.c_invoice_id 
 and (docstatus = ANY (ARRAY['CO'::bpchar, 'CL'::bpchar])) and c.c_doctype_id in
 (select c_doctype_id from c_doctype Where name in('AR Invoice Indirect Return - GST'))
 ) ) AS INT_totalbillcount,
...
#                  'customer_type': self.customer_type,
        ...
        res = self.env['ir.model.data'].check_object_reference(
                                            'ss_pos_reports', 'ss_view_bill_count_report')
        return {
                    'name': 'Bill No Count Report',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'res_model': 'ss.bill.count.report.wzd',
                    'domain': [],
...
class billcount_screen_wzd(models.Model):
    _name = "ss.bill.count.report.wzd"
    _description = "Bill No Count Reports"
    
    name=fields.Char(string="Name",default='Bill No Count Report')
    billno_count_line = fields.One2many('ss.billno.count.line','bill_id',string='Bill Count Line')
    ...
    def ss_print_bill_count_excel_report(self):
        ...
        sql = '''    
                   select terminal,startno ,endno,totalbillcount,onlinecnt,oflinecnt,cancelcount 
                   from
        ...
        fp =io.BytesIO()
        workbook.save(fp)
        export_id = self.env['ss.excel.extended.billcount.rep'].create({'excel_file': base64.encodestring(fp.getvalue()), 'file_name': filename})
...
class billno_count_screen_line(models.Model):
    _name = "ss.billno.count.line"
    _description = "Bill Count"
    
    bill_id = fields.Many2one('ss.bill.count.report.wzd',string='bill_id',ondelete='cascade')
...
class excel_extended_billno_count_rep(models.Model):
    _name= "ss.excel.extended.billcount.rep"
