...
#                 db_conn = self.env['db.connection'].search([('company_id','=',self.company.id)], limit=1)
        ...
        #                 sql +=" and pt.name ='%s'"%(product_id)                  
                    ...
                    sql='''
                      ...
                      select name,ad_org_id as org_id from um_brand
...
#                  'customer_type': self.customer_type,
        ...
        brand_id = self.env['brand.wzd'].create(vals)

        res = self.env['ir.model.data'].check_object_reference(
                                            'lifestyle_masters', 'brand_wizard')
        return {
                    'name': '',
                    'view_type': 'form',
                    'view_mode': 'form',
...
class brand_wzd(models.Model):
    _name = "brand.wzd"
    _description = "Brand Wizard"
    
    
    name = fields.Char(default="Brand Wizard")
    brand_line = fields.One2many('brand.master','attribute_id',string='Open Order Line')
    start_date = fields.Date('Date From') #, required=True
    end_date = fields.Date(string="Date To")
...
class BrandMaster(models.Model):
    ...
    attribute_id = fields.Many2one('brand.wzd',string='attribute_id',ondelete='cascade')
