...
class Customer(models.Model):
    ...
    # closing database connection.
        ...
        customer_id = self.env['customer.wzd'].create(vals)

        res = self.env['ir.model.data'].check_object_reference(
                                            'lifestyle_masters', 'customer_wizard')
        return {
                    'name': '',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'res_model': 'customer.wzd',
                    'domain': [],
...
class customer_wzd(models.Model):
    _name = "customer.wzd"
    _description = "customer Wizard"
    
    
    name = fields.Char(default="customer Wizard")
    customer_line = fields.One2many('customer.master','attribute_id',string='Open Order Line')
    start_date = fields.Date('Date From') #, required=True
    end_date = fields.Date(string="Date To")
...
class CustomerMaster(models.Model):
    ...
    attribute_id = fields.Many2one('customer.wzd',string='attribute_id',ondelete='cascade')
