...
class total_sales_details_wzd(models.Model):
    ...
    def print_totalsales_report(self):
...
and ('N'=%s or i.um_onlinesales='Y')
  ...
  ORDER BY sal.datetrx                                  
        ...
        for line in get_lines(self):
            if line['total_sales_amt']:
                total_sales_amt_tot += line['total_sales_amt']
            if line['dis_amt']:
                dis_amt_tot += line['dis_amt']
            if line['round_off']:
                round_off_tot += line['round_off']
            if line['tot_net_amt']:
                tot_net_amt_tot += line['tot_net_amt']
            if line['bill_count']:
                bill_count_tot += line['bill_count']
                         
            total_sales_line.append((0, 0, {
        ...
        res = self.env['ir.model.data'].check_object_reference(
                                            'ls_pos_reports', 'view_total_sales_screen_wzd_report')
        return {
                    'name': 'Total sales Report',
                    'view_type': 'form',
                    'view_mode': 'form',
                    'res_model': 'total.screen.wzd',
                    'domain': [],
...
class total_sales_screen_wzd(models.Model):
    _name = "total.screen.wzd"
    _description = "Total Sales Reports"
    
    name = fields.Char(default="Total Sales Report")
    total_sales_line = fields.One2many('total.sales.line', 'deptsale_id')
    start_date = fields.Date('Date From')
    end_date = fields.Date('Date To')
    company_id = fields.Char('Company')
    online_sales = fields.Char('Online Sales Only')
    
    def print_total_orders_excel_report(self):
        ...
        fp = io.BytesIO()
        workbook.save(fp)
        export_id = self.env['excel.extended.total.rep'].create({'excel_file': base64.encodestring(fp.getvalue()), 'file_name': filename})
...
total_sales_screen_wzd()

 
class total_sales_screen_line(models.Model):
    _name = "total.sales.line"
    _description = "Sales summary Line"
    
    deptsale_id = fields.Many2one('total.screen.wzd', string='deptsale_id', ondelete='cascade')
...
total_sales_screen_line()
    
     
class excel_extended_total_rep(models.Model):
    _name = "excel.extended.total.rep"
