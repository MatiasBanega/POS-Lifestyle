...
#     is_unusedbill = fields.Boolean(string='Show Unused Exchange Bill Only')
    ...
    def onchange_cashier(self):
        if self.organization_id:
            domain = {'cashier_id': [('org_id', '=',self.organization_id.org_id)]}
        else:
            domain = {'cashier_id': []}
        if domain:
            res = {}
...
#                 self.env['ss.pos.exchange.product.screen.line'].search([]).unlink()                
                ...
                if cashier_id:
                    sql='''
                        select i.documentno as str_ExchangeBill,
                            ...
                            il.qtyentered as num_ReturnQty,
                ...
                for row in pos_data:                
                    dict = {'str_ExchangeBill':row[0],'str_Originalbill':row[1] ,'dt_InvoiceDate':row[2],
                            ...
                            'num_ReturnQty':row[5] ,'NUM_OiginalInvoiceSP':row[6] ,
                            'num_linetotal':row[7] ,
                            'str_Cashier':row[8] ,'str_Terminal':row[9] ,}
...
#                'is_unusedbill' : self.is_unusedbill,
        ...
        res = self.env['ir.model.data'].check_object_reference(
                                            'ss_pos_exchange_report', 'ss_view_pos_order_wise_wzd_report')
        return {
...
#         is_unusedbill = self.is_unusedbill  or ''
        ...
        sheet.write(2, 5, 'Returnqty', format6)
...
#         sheet.write(2, 13, 'Balanceamt', format6)
        ...
        sql = '''
                 select exchange_bill,original_bill ,to_char(invoice_date,'dd/mm/yyyy'),product_code,product_name,
                 return_qty,original_invoicesp,line_total,cashier,terminal
                 from
...
class pos_product_screen_line(models.Model):
    ...
    return_qty = fields.Integer(string="Exchangeqty")
