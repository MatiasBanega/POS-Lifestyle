...
class FormView(models.Model):
    ...
    def get_data(self): 
        print('function')
        self.env['dept.wise.sales.view'].search([]).unlink()
        fetched_data=self.env['dept.wise.sales.line'].search([])
        if fetched_data:
            ...
            return {
                        'name':  'Department Wise Sales Form',
                        'view_type': 'form',
                        'view_mode': 'tree,form',
                        'res_model': 'dept.wise.sales.view',
                        'domain': [],
